...
- ✓ 1.5 Faction Dynamics (세력 형성·외교·전쟁·승계·와해)
- ✓ 1.6 World Impact (목표 달성 → 실제 마을·종교·왕국·상점)

**Step 1 전체 완료 ✓ — RebornNPC가 운영 수준에 도달. 다음: Step 2 (스킬 효과).**
...
## Step 2 — 스킬 효과 진짜 구현 (RebornSkill) ✓ 완료
**구현됨:** SkillType 11종을 YAML에서 자동 추론(136스킬 손 안 댐). EffectExecutor가
9종 효과(MELEE/PROJECTILE/AOE/HEAL/BUFF/DASH/BLINK/DOT/CHAIN/SUMMON)를 종류별로 실행.
Element 상성표(×1.5/×0.66)+몹 속성 추정+상태이상, 투사체 메타데이터 리스너,
캐스팅 바·피격 중단까지. (이하 원래 계획)

**과거 현 상태:** `SkillCaster.applyEffect()`가 스킬 종류와 무관하게 "정면 8블록 가장 가까운 1명
...
PHASE 1 (기반)
  ...
  Step 4  퀘스트     [✓ 완료 — 단계형 진행·6타입 연동·CUSTOM API]
  Step 2  스킬효과
  Step 3  몹 AI 10종
  Step 4  퀘스트 엔진
  Step 5  세계 디렉터 AI
...
**바로 다음:** Step 5 (세계 디렉터 AI) — "계속"이라고 하면 시작.
